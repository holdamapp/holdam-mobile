...
void main() {
  ...
  testWidgets("When the user tap the START button while the timer is already running the lauchTimer it's not called and the timer keeps increasing by one",
  ...
  testWidgets('When the user press the RESET button the timer should return to original value and not keep running',
      (WidgetTester tester) async {
        await tester.pumpWidget(const MyApp());
        final MyTimerState state = tester.state(find.byKey(timerKey).first);
        int initialTimerValue = state.timerValue;
        ...
        // ignore: todo
        // TODO:  por alguyn motivo se esta renderizando eln los test dos veces el button RESET,
        // y por eso tuve que agregarel `.last` al final por que habia mas de uno. INVESTIGAR
